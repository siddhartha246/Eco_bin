...
export default function RewardsPage() {
  ...
  const handleRedeemReward = async (rewardId: number) => {
    ...
    if (reward && balance >= reward.cost && reward.cost > 0) {
      try {
        ...
        await createTransaction(dbUser.id, 'redeemed', reward.cost, `Redeemed ${reward.name}`);
  ...
  const handleRedeemAllPoints = async () => {
    ...
    if (balance > 0) {
      try {
        ...
        // Create a new transaction record
        await createTransaction(dbUser.id, 'redeemed', balance, 'Redeemed all points');
  ...
  const refreshUserData = async () => {
    if (dbUser) {
      const fetchedUser = await getUserByEmail(dbUser.email);
      if (fetchedUser) {
        const fetchedTransactions = await getRewardTransactions(fetchedUser.id);
        setTransactions(fetchedTransactions as Transaction[]);
        const fetchedRewards = await getAvailableRewards(fetchedUser.id);
