...
export default function CollectPage() {
    ...
    const fetchUserAndTasks = async () => {
      ...
      try {
        ...
        if (clerkUser && clerkUser.primaryEmailAddress?.emailAddress) {
          ...
          toast.error('Please log in to access waste collection tasks.')
          return
        }
  ...
  const handleVerify = async () => {
    if (!selectedTask || !verificationImage || !dbUser) {
    ...
    setTimeout(async () => {
      // For testing purposes, always return successful verification
      const mockResult = { wasteTypeMatch: true, quantityMatch: true, confidence: 0.85 };
      ...
      if (mockResult.wasteTypeMatch && mockResult.quantityMatch && mockResult.confidence > 0.7) {
        ...
        await saveCollectedWaste(selectedTask.id, dbUser.id, mockResult);
            ...
            {paginatedTasks.map(task => (
              <div key={task.id} className="bg-white p-4 rounded-lg shadow-sm border border-gray-200">
                ...
                <div className="flex justify-end">
                  ...
                  {task.status === 'in_progress' && task.collectorId === dbUser?.id && (
